    ...
    <Popover sounds>
      <div className="group/buttons relative flex rounded-lg bg-secondary *:data-[slot=button]:focus-visible:relative *:data-[slot=button]:focus-visible:z-10">
        ...
        <DropdownMenu sounds>
          ...
          <DropdownMenuContent
            align="end"
            className="w-fit animate-none! rounded-lg shadow-none [&_a]:inline-flex [&_a]:items-center [&_a]:gap-1.5 [&_a]:whitespace-nowrap"
          >
            {MENU_ITEMS.map(([key, render]) => (
              <DropdownMenuItem key={key} sound="click">
                {render(url)}
              </DropdownMenuItem>
            ))}
          </DropdownMenuContent>
